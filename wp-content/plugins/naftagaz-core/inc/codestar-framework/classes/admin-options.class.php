...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public $args         = array(
      ...
      // framework title
      'framework_title'         => 'پوسته دانشجویار<small style="margin-right: 5px;">version : 1.0.0</small>',
      'framework_class'         => '',
    ...
    public function set_options( $ajax = false ) {
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! $ajax && ! empty( $response[ 'csf_import_data' ] ) ) {
          ...
          $importing    = true;
          $this->notice = esc_html__( 'تنظیمات با موفقیت وارد شد.', 'csf' );
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          $this->notice = esc_html__( 'تنظیمات پیش فرض بازیابی شد.', 'csf' );
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( 'شما تغییرات ذخیره نشده ای دارید، تغییرات خود را ذخیره کنید!', 'csf' ) .'</div>' : '';

            echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( 'نمایش تمام تنظیمات', 'csf' ) .'"><i class="fas fa-outdent"></i></div>' : '';
            ...
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'ذخیره تنظیمات', 'csf' ) .'" data-save="'. esc_html__( 'درحال ذخیره سازی...', 'csf' ) .'">';
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'بازنشانی بخش', 'csf' ) .'" data-confirm="'. esc_html__( 'آیا مطمئن هستید که گزینه های این بخش را بازنشانی می کنید؟', 'csf' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'بازنشانی همه', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( 'آیا مطمئن هستید که می خواهید همه تنظیمات را به مقادیر پیش فرض بازنشانی کنید؟', 'csf' ) .'">' : '';
            echo '</div>';
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'ذخیره تنظیمات', 'csf' ) .'" data-save="'. esc_html__( 'درحال ذخیره سازی...', 'csf' ) .'">';
          echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'بازنشانی بخش', 'csf' ) .'" data-confirm="'. esc_html__( 'آیا مطمئن هستید که گزینه های این بخش را بازنشانی می کنید؟', 'csf' ) .'">' : '';
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'بازنشانی همه', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( 'آیا مطمئن هستید که می خواهید همه تنظیمات را به مقادیر پیش فرض بازنشانی کنید؟', 'csf' ) .'">' : '';
          echo '</div>';
